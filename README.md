...
本插件是这个插件的扩展+重构 从Nonebot换为Nonebot2 添加了一些新的指令，并给所有指令都加入了详细说明
...
https://github.com/TurboServlet/nonebot-plugin-turbobot

支持文档

https://github.com/TurboServlet/turboNET-doc

https://api.sys-allnet.com/swagger-ui/index.html
...
-# 插件允许监听的群组
 ...
 session_expire_timeout: int = 60  
 
 -# 修改头像等api发送请求后 等待用户上传头像的超时时间

 arcade_info_groups: List[int] = [] 
 
 -#/info 系列机厅人数查询功能的单独权限管理，权限在allowed_groups下，用于让您在大群开启查询人数功能的同时保证其他指令不在大群被触发

 arcade_info_public_token: int = 0/1 
 
 -#此功能可以开启/info系列指令的权限，1=使用公共token（无需绑定），0=使用用户私有token（需绑定）
